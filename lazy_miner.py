...
def dfg_calculate_with_pandas(configuration, data):
    ...
    activities = numpy.sort(numpy.unique(data[configuration.activity_name].values))
    ...
    for k in numpy.arange(m):
        ...
        #i, j = activities.tolist().index(timers_edges[k, 1]), activities.tolist().index(timers_edges[k, 0])
